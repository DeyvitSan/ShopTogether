...
fun RegisterScreen(onSuccess: () -> Unit, onBack: () -> Unit, vm: AuthViewModel = hiltViewModel()){
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbar) },
        topBar = { TopAppBar(title = { Text("Crear cuenta") }, navigationIcon = { IconButton(onClick = onBack) { Icon(Icons.AutoMirrored.Filled.ArrowBack, null) } }) }
    ) { pad ->
        ...
        Column(Modifier.fillMaxSize()
            ...
            Button(onClick = { vm.register(name, email, password) },
                ...
                if (uiState.isLoading) CircularProgressIndicator(Modifier.size(20.dp),
                    strokeWidth = 2.dp) else Text("Registrarse")
            }
        }
    }
}